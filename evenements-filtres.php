...
<!-- Partie EVENEMENTS -->
      ...
      if($_GET["tri"] == "Aujourd'hui"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `date` = CURRENT_DATE AND `statut`= \"publié\" ORDER BY `evenement`.`heure` ASC");
      ...
      if($_GET["tri"] == "pour les 7 jours à venir"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `date` BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL 6 DAY) AND `statut`= \"publié\" ORDER BY `evenement`.`date`  ASC");
          $valeurs= $resultatCompte->fetch();
          $nbentree = $valeurs['nbentree'];
      ...
      if($_GET["tri"] == "pour les 30 jours à venir"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `date` BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL 29 DAY) AND `statut`= \"publié\" ORDER BY `evenement`.`date`  ASC");
          $valeurs= $resultatCompte->fetch();
          $nbentree = $valeurs['nbentree'];
      ...
      if($_GET["tri"] == "trié(s) par département"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `statut`= \"publié\" ORDER BY `departement` ASC");
          $valeurs= $resultatCompte->fetch();
          $nbentree = $valeurs['nbentree'];
      ...
      if($_GET["tri"] == "trié(s) par catégorie"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `statut`= \"publié\" ORDER BY `categorie` ASC");
          $valeurs= $resultatCompte->fetch();
          $nbentree = $valeurs['nbentree'];
      ...
      if($_GET["tri"] == "trié(s) par public"){
          $resultatCompte = $bdd->query("SELECT COUNT(id) AS nbentree FROM evenement WHERE `statut`= \"publié\" ORDER BY `public` ASC");
          $valeurs= $resultatCompte->fetch();
          $nbentree = $valeurs['nbentree'];
